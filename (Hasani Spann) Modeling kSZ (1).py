...
# ### Obtaining dK_bar/dZ (disregard for now)
# 
...
#    dC_kSZ_dz = ion_kSZ_integrand(l_s, kSZ_Bat, z_rei, sigma_rei)+mykSZ.ksz_integrand(z_rei, new_l)
  #  dK_bar_integrand = []
...
#dK_bar_int = dK_bar_integrand(new_l, Ws, kSZ_Bat, z_rei, sigma_rei)
...
for i in range(1, 1101):
    Z.append(i)
    #note: use numpy.arange instead next time

for l_int in range (1100, 2550, 290):
    late_time_kSZ_integrand.append(mykSZ.ksz_integrand(Z,l_int))
    
l_int = np.arange(1100, 2550, 290)
...
plt.title("Late-Time kSZ Integrand \n(Over redshift z and frequency l)")
plt.xlabel("l")
plt.ylabel("z")
plt.contourf(X, Y, a, 100, cmap='RdGy')
plt.colorbar();
