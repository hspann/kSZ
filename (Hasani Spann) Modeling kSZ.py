...
plt.plot(ls, mykSZ.get_cl(ls)*ls*(ls+1)/(2*np.pi), label=r'Late-Time kSZ')
new_l = np.arange(1100,2550) # new ell values, for example
...
def ion_kSZ_integrand(z, kSZ_Bat, z_rei, sigma_rei):
    ...
    return ((np.exp((-(z-z_rei)**2)/(2*sigma_rei**2))/((2*np.pi*sigma**2)**(1/2)))*kSZ_Bat)
...
for i in range(np.size(Dl_kSZ_battaglia_interpolated_numpy)):
    ...
    ion_kSZ_intermediate.append(integrate.quad(ion_kSZ_integrand, z_min, z_max, args=(kSZ_Bat,z_rei,sigma_rei)))
...
ax[0,0].set_title(r'Tot CMB Spectrum (muK Units?)')
ax[0,1].plot(ells, mykSZ.get_cl(ells), color= 'r');
ax[0,1].set_title(r'General kSZ Spectrum (muK^2 Units)')
ax[1,0].plot(ls, ((totCL[:,0])**(1/2))/(mykSZ.get_cl(ls)), color='k')
...
# ### Obtaining dK_bar/dZ
# 
# Note: At the moment, this is flawed, as the result should be expressed as a variable function. I'll have to use sympy or another repository that utilizes algebraic variables to resolve this integral.

# In[ ]:


def ion_kSZ_integrand(z, kSZ_Bat, z_target, sigma):
    #z and sigma are constants, kSZ_Bat is the kSZ from Battaglia
    return ((np.exp((-(z-z_target)**2)/(2*sigma**2))/((2*np.pi*sigma**2)**(1/2)))*kSZ_Bat)
#NOTE: You must add both this and the late time kSZ integrand (mykSZ.ksz_integrand()) in order to get dC_l^kSZ/dz
#maybe make a call to integral calculator

def dK_bar_integrand(l_s, W, kSZ_Bat, z_rei, sigma_target):
    dC_kSZ_dz = ion_kSZ_integrand(z, kSZ_Bat, z_rei, sigma_rei)+mykSZ.ksz_integrand(z, new_l)
    dK_bar_integrand = []
    for i in range(len(dC_kSZ_dz)):
        dK_bar_integrand.append(dC_kSZ_dz[i]*(W[i]**2)*l_s[i]/(2*np.pi))
    return dK_bar_integrand 
...
dK_bar_int = dK_bar_integrand(new_l, Ws, kSZ_Bat, z, sigma)

dK_bar = np.trapz(dK_bar_int)

print(dK_bar)
print(mykSZ.ksz_integrand([1,2,3,4,5], 1000))
...
kSZ_integrand = []
for Z in range(1, 1100):
    l_dependent = [] 
    for l_int in range (1450):
        l_dependent.append(mykSZ.ksz_integrand(Z,l_int))
    kSZ_integrand.append(l_dependent)


# In[18]:


print(len(kSZ_integrand))
...
z = Symbol('z')
print(mykSZ.ksz_integrand(z, new_l))
